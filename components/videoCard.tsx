...
const VideoCard: React.FC<VideoCardProps> = ({video, onDownload}) => {
    ...
    // Memoize URL generating functions to prevent unnecessary recalculations
    ...
    // Reset errors when hovering changes
    useEffect(() => {
        if (isHovered) {
    ...
    return (
        ...
        >
            <figure className="aspect-video relative overflow-hidden">
                ...
                {isHovered && !isVideoLoaded && !previewError && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 z-10">
                        <div className="loading loading-spinner loading-md"></div>
                    </div>
                )}
                
                {/* Preview video when hovered */}
                {isHovered ? (
                    previewError ? (
